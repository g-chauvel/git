...
int cmd__delta_base_cache(int argc, const char **argv)
{
	 ...
	 * released all resources owned by first. Retarget first at the second
	 * pack, whose path has the same length, then reopen its index. This keeps
	 * the packed_git pointer stable without copying its internal fields.
	 */
