...
int cmd__delta_base_cache(int argc, const char **argv)
{
	...
	first_base = delta_base_offset(first, &first_oid);
	second_base = delta_base_offset(second, &second_oid);
	...
	/* Reuse the same address, independent of the system allocator. */
	*first = *second;
