...
int cmd__delta_base_cache(int argc, const char **argv)
{
	...
	/*
	 * Reuse the same address, independent of the system allocator. This is
	 * deliberately a test-only ownership transfer: close_pack(first) has
	 * released all resources owned by first, while second is about to be
	 * discarded. List heads are self-referential, so reinitialize the one
	 * embedded in the copied structure after transferring the fields.
	 */
	memcpy(first, second, sizeof(*second) + strlen(second->pack_name) + 1);
